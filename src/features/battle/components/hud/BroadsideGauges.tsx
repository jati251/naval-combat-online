...
export const BroadsideGauges: React.FC<BroadsideGaugesProps> = React.memo(({
  ...
  return (
    <div className="pointer-events-auto flex items-center gap-3.5 naval-plaque px-4 py-2 select-none rounded-md border border-amber-600/40 shadow-xl relative">
      ...
      <div className={`flex flex-col gap-1 w-24 transition-opacity ${isAimingStbd ? 'opacity-40' : 'opacity-100'}`}>
        <div className="flex items-center justify-between text-[10px] font-mono">
          <span className={`font-cinzel font-bold tracking-wider ${isAimingPort ? 'text-amber-300 gold-emboss' : 'text-stone-400'}`}>
            PORT [Q]
          </span>
          <span className={`text-[9px] font-mono font-bold ${isPortReady ? 'text-emerald-400' : 'text-amber-400/90'}`}>
      ...
      <div className={`flex flex-col gap-1 w-24 transition-opacity ${isAimingPort ? 'opacity-40' : 'opacity-100'}`}>
        <div className="flex items-center justify-between text-[10px] font-mono">
          <span className={`font-cinzel font-bold tracking-wider ${isAimingStbd ? 'text-amber-300 gold-emboss' : 'text-stone-400'}`}>
            STBD [E]
          </span>
          <span className={`text-[9px] font-mono font-bold ${isStbdReady ? 'text-emerald-400' : 'text-amber-400/90'}`}>
