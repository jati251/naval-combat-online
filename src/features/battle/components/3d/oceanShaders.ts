export const getOceanVertexShader = (waveShaderChunk: string) => `
        ...
        void main() {
          ...
          // Compute coast proximity in vertex shader (saves millions of ops per frame in frag)
          float minDistToShore = 9999.0;
          for (int i = 0; i < 12; i++) {
            if (uIslandPos[i].z <= 0.0) continue;
            vec2 relPos = displaced.xz - uIslandPos[i].xy;
            float sandR = uIslandPos[i].z;
            float centerDistSq = dot(relPos, relPos);
            float maxInfluence = (sandR * 1.8 + 64.0);
            if (centerDistSq > maxInfluence * maxInfluence) continue;
          ...
          vMinDistToShore = minDistToShore;

          gl_Position = projectionMatrix * viewMatrix * vec4(displaced, 1.0);
        }
`;

export const getOceanFragmentShader = () => `
        uniform float uTime;
        ...
        void main() {
          ...
          float minDistToShore = vMinDistToShore;
